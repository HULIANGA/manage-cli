    ...
    methods: {
      ...
      async toggleExpend(params) { // 展开收缩
        if (params.row.expend === 'expend') { // 收缩
          this.tableData = this.tableData.filter(obj => obj.pId !== params.row.ad_operator_id);
          ...
          const requestParams = {
            ...
            ...this.filterParams,
            ...this.sortParams,
            ad_operator_id: params.row.ad_operator_id
          };
          const result = await getEfApp(requestParams);
          const { data } = result;
          if (data.code === 0) {
            this.tableData.splice(params.index + 1, 0, ...data.data.items.map(item => ({
              ...item,
              ad_operator: item.app_name,
              isChild: true,
              pId: params.row.ad_operator_id
            })));
          }
